...
$giLines = @(
    ...
    ''
    '# Dokumen kerja yang di sisi backend juga gitignored - disalin manual lewat'
    '# setup-android-project.ps1, bukan lewat repo ini.'
    'docs/android-build-phases.md'
    'docs/android-todo-rbac-angkatan.md'
    ''
    '# Android / Gradle'
    '.gradle/'
    'build/'
...
Write-Host "  3. Buat kerangka project (SEKALI): Android Studio -> New Project ->"
Write-Host "     Empty Activity/Compose, package id.sch.sman1tanjung.sim, minSdk 24,"
Write-Host "     Kotlin DSL, lokasi = folder ini. Folder harus KOSONG saat wizard"
Write-Host "     jalan -> pindahkan CLAUDE.md + docs/ sementara, kembalikan setelah."
Write-Host "     Pastikan Run berhasil dulu (bukti toolchain sehat)."
Write-Host "  4. Buka folder ini di VS CODE -> Claude Code -> Fase 1 (Plan Mode):"
