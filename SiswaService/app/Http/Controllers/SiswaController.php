...
class SiswaController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($modeFoto) {
                $ids = array_column($pageArr['data'], 'idSiswa');
                $punya = empty($ids)
                    ? collect()
                    : \Illuminate\Support\Facades\DB::table('siswas')->whereIn('id', $ids)->pluck('foto', 'id');

                $pageArr['data'] = array_map(function ($row) use ($punya) {
                    $row['punyaFoto'] = !empty($punya[$row['idSiswa'] ?? null] ?? null);
                    return $row;
                }, $pageArr['data']);
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validate = Validator::make($request->all(), [
                ...
                'email'        => ['required', 'email', \Illuminate\Validation\Rule::unique('siswas', 'email')->whereNull('deleted_at')],
            ...
            //
            // `siswas.email` unik di level DB dan model memakai soft delete, jadi
            ...
            $dipulihkan = false;
            $siswa = Siswa::onlyTrashed()->where('email', $request->email)->first();
            if ($siswa) {
                $siswa->restore();
                $siswa->update($data);
                $dipulihkan = true;
            ...
            return $this->response(
                $dipulihkan
                    ? "Data Siswa dipulihkan dari record yang sebelumnya dihapus."
                    : "Data Siswa berhasil disimpan.",
                Response::HTTP_CREATED,
                ['idSiswa' => $siswa->id, 'dipulihkan' => $dipulihkan]
            );
        } catch (Exception $e) {
    ...
    public function update(Request $request)
    {
        try {
            ...
            $pesanSukses = "Siswa dengan id:{$request->idSiswa} berhasil diupdate.";
            if ($request->filled('email')
                && strcasecmp(trim((string) $request->email), (string) $siswa->email) !== 0) {
    ...
    public function nama(Request $request)
    {
        try {
            ...
            $q = Siswa::withTrashed()->select('id', 'nama_lengkap');
