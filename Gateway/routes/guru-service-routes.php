...
Route::middleware(['auth:api', 'force.pwd'])->prefix(config('gateway.guru_prefix'))->group(function(){
    ...
    Route::post('kartu/terbitkan', [GuruController::class, 'terbitkanKartu'])->middleware('check.role:SuperAdmin,Admin,AdminSekolah');
    Route::post('kartu/blokir', [GuruController::class, 'blokirKartu'])->middleware('check.role:SuperAdmin,Admin,AdminSekolah');
    Route::delete('/{id}', [GuruController::class, 'destroy'])->middleware('check.role:SuperAdmin,Admin,AdminSekolah');
});
